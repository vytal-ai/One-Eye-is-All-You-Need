...
class gazetrack_dataset(Dataset):
    def __init__(self, root, phase='train', size=(128, 128), transform=True, v=True):
        ...
        self.side = [1 for i in range(int(len(self.files)/2))] + [0 for i in range(len(self.files) - int(len(self.files)/2))]
        assert len(self.side) == len(self.files), "len(self.side) != len(self.files)"
        self.side = shuffle(self.side, random_state=42)
    ...
    def __getitem__(self, idx):
        ...
        if self.side[idx] == 1:
            ...
            return self.files[idx], l_eye, kps, out, screen_w, screen_h
        else:
            r_eye = self.aug(r_eye)
            return self.files[idx], r_eye, kps, out, screen_w, screen_h
