...
class eye_model(nn.Module):
  ...
  def forward(self, x):
    ...
    block2_out = self.block2_4(torch.cat([self.block2_0(block1_out), self.block2_1(block1_out), self.block2_2(block1_out), self.block2_3(block1_out)], 1))
    block2_out = self.block2_activation(block2_out + block1_residual)
    block2_residual = self.block2_downsample(block2_out)

    block3_out = self.block3_4(torch.cat([self.block3_0(block2_out), self.block3_1(block2_out), self.block3_2(block2_out), self.block3_3(block2_out)], 1))
    block3_out = self.block3_activation(block3_out + block2_residual)
